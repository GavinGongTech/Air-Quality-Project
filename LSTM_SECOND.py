# The first LSTM model is a good starting point. We treated each sensor independently, which is reasonable given the limited data per sensor. 
# However, this approach may not fully capture the relationships between different sensors. In the next iteration, we will 
# explore adding another input which is a lagged PM2.5 factor, and see if this makes the model perform better here. 

# So now input becomes the PM2.5 and meteorological features across the hours
# Output becomes the target PM2.5 at time t + h
...
def train_per_sensor_lstm(df: pd.DataFrame, cfg: Config):
    ...
    # Create an explicit input copy so it's clear we're using *past* PM2.5 as a feature
